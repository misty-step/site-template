...
export interface DesignGenome {
  heroStyle?: 'fullbleed' | 'split' | 'minimal' | 'video-bg';
  navStyle?: 'transparent' | 'solid' | 'sidebar';
  galleryStyle?: 'grid-2' | 'grid-3' | 'masonry' | 'carousel';
  testimonialStyle?: 'cards' | 'quotes' | 'slider';
  ctaStyle?: 'banner' | 'floating' | 'inline';
  footerStyle?: 'minimal' | 'full' | 'mega';
  sectionSpacing?: 'tight' | 'normal' | 'airy' | 'dramatic';
  maxWidth?: 'narrow' | 'standard' | 'wide' | 'full';
  roundness?: 'none' | 'subtle' | 'rounded' | 'pill';
  shadowStyle?: 'none' | 'subtle' | 'medium' | 'dramatic';
  animationLevel?: 'none' | 'subtle' | 'moderate';
  imageTreatment?: 'raw' | 'rounded' | 'masked' | 'overlay';
}
...
export interface SiteConfig {
  ...
  // Design genome for evolutionary design engine
  // When not set, defaults to current behavior (backward compatible)
